        ...
        bot.on('message', async (msg) => {
            ...
            if (text === "👥 Check Referrals") {
                ...
                if (referredUsers.length === 0) {
                    bot.sendMessage(chatId, `ℹ️ Abhi tak koi referral record nahi mila hai.`);
                    return;
                }
...
async function sendCustomerHomeMenu(bot, chatId) {
    ...
    const referralLink = `https://t.me/${primaryCustomerBotUsername}?start=ref_${chatId}`;
    
    const welcomeMessage = `✨ **Welcome to JPW REACHED SERVICES BOT!** 🚀\n\n🆔 **Aapki Web Login ID / Chat ID:** \`${chatId}\`\n\n🔗 **Direct Mini App Portal Link:**\n${portalUrl}\n\n👥 **Aapka Refer & Earn Link:**\n\`${referralLink}\`\n*(100 se 0.0001 Reach tak kuch bhi mil sakta hai - For Marketing Purpose! Jab friend ₹100+ ka recharge karega)*\n\n📝 **Order Format:** Order dene ke liye seedhe chat mein bhejein: \`TARGET_ID PASSWORD\`\n\n📦 **Aapka Active Package:** ${activePkg}\n💎 **Remaining Reaches:** ${bal} Reaches\n\n🤖 *JPW REACHED SERVICES BOT*\n💻 *Developed by tenaga technology*`;
    ...
    try {
        ...
        bot.on('callback_query', async (query) => {
            ...
            if (data === 'show_referral_info') {
                ...
                bot.sendMessage(chatId, `👥 **Refer & Earn System (100 se 0.0001 Reach tak):**\n\nAapka Link:\n\`${referralLink}\`\n\n📌 **Rule:** 100 se 0.0001 Reach tak kuch bhi mil sakta hai! Jab aapka friend is link se judkar **minimum ₹100** ka pehla recharge karega, tab aapko **0.0001 se 1 Reach** tak ka random bonus milega (2% chance hai ki aapko poora 1 Reach mil jaye)!`, { parse_mode: 'Markdown' });
            ...
            if (amount >= 100 && user.referredBy && !user.hasRecharged100) {
                ...
                if (referrer && !user.referralRewarded) {
                    ...
                    try {
                        if (CUSTOMER_BOT_TOKENS[0]) {
                            const tempBot = new TelegramBot(CUSTOMER_BOT_TOKENS[0], { polling: false });
                            await tempBot.sendMessage(referrer.telegramChatId, `👥 **Referral Bonus Received (Marketing Bonus)!** 🎉\n\nAapke referred user ne ₹100+ ka recharge kar liya hai!\nAapko mile hain: \`+${refBonus.toFixed(4)} Reaches\`! 💎\n\n*(JPW REACHED SERVICES BOT)*`, { parse_mode: 'Markdown' });
                        }
                    } catch(e) {}
                }
            }
